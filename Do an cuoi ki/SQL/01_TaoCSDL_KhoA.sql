/* ============================================================================
   ...
   ----------------------------------------------------------------------------
   SITE  S1 : Kho Trung tâm
   INSTANCE : MIGNON\KHO_A
   DATABASE : KhoA
   MẢNH     : TonKho_A, PhieuNhap_A, ChiTietNhap_A,
              PhieuXuat_A, ChiTietXuat_A

   CHẠY TẠI : CHỈ chạy file này trên instance MIGNON\KHO_A
   ============================================================================ */

USE master;
GO

IF DB_ID('KhoA') IS NOT NULL
BEGIN
    ALTER DATABASE [KhoA] SET SINGLE_USER WITH ROLLBACK IMMEDIATE;
    DROP DATABASE [KhoA];
END
GO
...
/* ----------------------------------------------------------------------------
   PHẦN 2. MẢNH NGANG NGUYÊN THỦY  -  TonKho_A = σ(MaKho='KHO_A')(TonKho)
...
CREATE TABLE TonKho (
    ...
    CONSTRAINT CK_TonKho_Manh  CHECK (MaKho = 'KHO_A'),   -- <<< ĐỊNH NGHĨA MẢNH
...
/* ----------------------------------------------------------------------------
   PHẦN 3. CHỨNG TỪ
     PhieuNhap_A / PhieuXuat_A : phân mảnh ngang NGUYÊN THỦY theo MaKho
     ChiTietNhap_A / ChiTietXuat_A : phân mảnh ngang DẪN XUẤT (nửa nối
        theo MaPN / MaPX) - vì hai bảng này KHÔNG có thuộc tính MaKho

   Mã phiếu nhúng ký tự kho ('PNA0001') để khóa chính không đụng nhau giữa 3 site.
   ---------------------------------------------------------------------------- */
...
CREATE TABLE PhieuNhap (
    ...
    CONSTRAINT CK_PN_Manh  CHECK (MaKho = 'KHO_A'),        -- <<< ĐỊNH NGHĨA MẢNH
    CONSTRAINT CK_PN_MaSo  CHECK (MaPN LIKE 'PNA[0-9][0-9][0-9][0-9]'),
...
CREATE TABLE PhieuXuat (
    ...
    NguoiLap  NVARCHAR(50)  NOT NULL,
    TrangThai VARCHAR(12)   NOT NULL DEFAULT 'HOAN_TAT',
    CONSTRAINT CK_PX_Manh CHECK (MaKho = 'KHO_A'),         -- <<< ĐỊNH NGHĨA MẢNH
    CONSTRAINT CK_PX_MaSo CHECK (MaPX LIKE 'PXA[0-9][0-9][0-9][0-9]'),
...
CREATE TABLE PhieuDieuChuyen (
    ...
    CONSTRAINT CK_DC_LienQuan  CHECK (MaKhoNguon = 'KHO_A' OR MaKhoDich = 'KHO_A')
);
GO
...
INSERT INTO PhieuNhap (MaPN, MaKho, MaNCC, NgayNhap, NguoiLap) VALUES
 ('PNA0001', 'KHO_A', 'NCC01', '2026-08-05 08:30', N'Nguyễn Văn Hùng');
INSERT INTO ChiTietNhap (MaPN, MaVT, SoLuong, DonGia) VALUES
 ('PNA0001', 'VT001', 200, 95000),
 ('PNA0001', 'VT002', 80, 185000);

INSERT INTO PhieuNhap (MaPN, MaKho, MaNCC, NgayNhap, NguoiLap) VALUES
 ('PNA0002', 'KHO_A', 'NCC04', '2026-08-12 09:15', N'Nguyễn Văn Hùng');
INSERT INTO ChiTietNhap (MaPN, MaVT, SoLuong, DonGia) VALUES
 ('PNA0002', 'VT009', 300, 45000),
 ('PNA0002', 'VT010', 40, 350000);

GO

-- Phiếu xuất
INSERT INTO PhieuXuat (MaPX, MaKho, NgayXuat, NguoiNhan, NguoiLap) VALUES
 ('PXA0001', 'KHO_A', '2026-08-20 09:00', N'Công trình Vincom Bà Triệu', N'Phạm Thu Hà');
INSERT INTO ChiTietXuat (MaPX, MaVT, SoLuong, DonGia) VALUES
 ('PXA0001', 'VT001', 50, 98000),
 ('PXA0001', 'VT003', 3000, 1600);

INSERT INTO PhieuXuat (MaPX, MaKho, NgayXuat, NguoiNhan, NguoiLap) VALUES
 ('PXA0002', 'KHO_A', '2026-08-25 13:40', N'Công trình Chung cư Green Park', N'Phạm Thu Hà');
INSERT INTO ChiTietXuat (MaPX, MaVT, SoLuong, DonGia) VALUES
 ('PXA0002', 'VT009', 120, 48000);
...
PRINT N'===== SITE KHO_A / DATABASE KhoA =====';
